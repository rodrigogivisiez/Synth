 ...
 */
public class RangeBarPrecent extends RelativeLayout {
    ...
    public RangeBarPrecent(Context context) {
        super(context);
        mContext = context;
        init();
    }

    public RangeBarPrecent(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        init();
    }
    ...
    private void init() {
        ...
        View view = inflater.inflate(R.layout.range_bar_precent, this, true);
        WIDTH = 0;
    ...
    public void setProgress2(double progress) {
        mProgress2 = progress;
        if (mFormater != null)
            tvRight.setText(mFormater.getRight(progress));
        if (WIDTH > 0) {
    ...
    public void setProgress1(double progress) {
        mProgress1 = progress;
        if (mFormater != null)
            tvLeft.setText(mFormater.getLeft(progress));
        if (WIDTH > 0) {
