...
public class Piano extends View {
    ...
    public Piano(Context context, AttributeSet attrs, int defStyle) {
        ...
        int black_key_resource_id = R.drawable.key_black_custom;
        int white_key_resource_id = R.drawable.key_white_custom;
        int key_count = defaultKeyCount;

        if (attrs != null) {
            TypedArray a = getContext().obtainStyledAttributes(attrs, R.styleable.PianoView, 0, 0);
            black_key_resource_id = a.getResourceId(R.styleable.PianoView_blackKeyDrawable, R.drawable.key_black_custom);
            white_key_resource_id = a.getResourceId(R.styleable.PianoView_whiteKeyDrawable, R.drawable.key_white_custom);
    ...
    private class KeyPressListener implements OnTouchListener {
        ...
        private void pushKeyDown(MotionEvent event) {
            ...
            try {
                ...
                if (event.getPointerCount() >= event.getActionIndex() + 1) {
                    int pointer_index = event.getPointerId(event.getActionIndex());

                    int action = event.getAction() & MotionEvent.ACTION_MASK;
                    int pointerIndex = (event.getAction() & MotionEvent.ACTION_POINTER_ID_MASK) >> MotionEvent.ACTION_POINTER_ID_SHIFT;
                    int pointerId = event.getPointerId(pointerIndex);
                    Key key = isPressingKey(event.getX(pointerId), event.getY(pointerId));

                    if (!fingers.containsKey(pointerId)) {
                        Finger finger = new Finger();
                        finger.press(key);
                        fingers.put(pointerId, finger);
                    }
                }
            } catch (Exception e) {
        ...
        private void handlePointerIndex(int index, MotionEvent event) {
            ...
            if (key == null) {
                finger.lift();
            } else if (finger != null && finger.isPressing(key) == false) {
                finger.lift();
                finger.press(key);
            }
