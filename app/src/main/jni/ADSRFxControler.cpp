...
void ADSRFxControler::proccess() {
    ...
    for (map<string, ADSRFx *>::iterator iter = mADSRSend.begin();
         iter != mADSRSend.end(); ++iter) {
        ADSRFx *adsrFx = iter->second;
        mListener->setFx(adsrFx->keyFx, adsrFx->keyFxParam,
                         adsrFx->start + val * (adsrFx->end - adsrFx->start));
    }

}

void ADSRFxControler::addFx(string index, int keyFx, int keyFxParam, double start, double end,
                            bool isUpdate) {
    if (!isUpdate) {
