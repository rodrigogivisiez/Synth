...
void PianoRoll::process() {
    if (mIsPlay) {
        ...
        for (Midi *node = mHead; node != NULL; node = node->next) {
            ...
            if (isNeedPlay(node)) {
                __android_log_print(ANDROID_LOG_DEBUG, "pianondk",
                                    "midi start %f midi end %f midi play %s position %f",
                                    node->mStart, node->mEnd, node->mIsPlay ? "true" : "false",
                                    mPositionMs);
...
int PianoRoll::addMidi(int key, float start, float end) {
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "pianondk",
                        "midi start %f midi end %f midi play %s position %f", midi->mStart,
                        midi->mEnd, midi->mIsPlay ? "true" : "false", mPositionMs);
...
void PianoRoll::setStartEndLoopPositionPercent(double start, double end) {
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "pianostartend", "start = %d end = %d",
                        (int) mStartLoopPositionBeat, (int) mEndLoopPositionBeat);
