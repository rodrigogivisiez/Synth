...
int SeqControler::getIndexOfSeq() {
    return (int) mCurrentTime >= stepsArr[(int) (mPatterns[mCurrentIndex]->stepDur * 5)] * bpmToMs()
           ? 1 : 0;
...
float *SeqControler::proccess(Channel *channel, double currentPosition, double pitch) {
    ...
    if (isNext) {
        ...
        __android_log_print(ANDROID_LOG_DEBUG, "currentPosition", "dif %f index=%d", mCurrentTime,
                            mCurrentIndex);
    ...
    if (isNext) {
        for (int i = 0; i < 2; ++i) {
            for (map<string, CurveFx *>::iterator iter = curveSend[i].begin();
                 iter != curveSend[i].end(); ++iter) {
                CurveFx *cfx = iter->second;
                mListener->setFx(cfx->keyFx, cfx->keyFxParam, cfx->start +
                                                              mPatterns[mCurrentIndex]->curve[i] *
                                                              (cfx->end - cfx->start));
            }
        }
    }
...
void SeqControler::sendFxToCurve(string indexString, int curve, int fxType, int keyEffectParam,
...
void SeqControler::removeAll() {
    ...
    for (map<string, CurveFx *>::iterator iter = curveSend[0].begin();
         iter != curveSend[0].end(); ++iter) {
        CurveFx *cfx = iter->second;
        delete (cfx);
    }
    curveSend[0].clear();
    for (map<string, CurveFx *>::iterator iter = curveSend[1].begin();
         iter != curveSend[1].end(); ++iter) {
        CurveFx *cfx = iter->second;
        delete (cfx);
    }
    curveSend[1].clear();
