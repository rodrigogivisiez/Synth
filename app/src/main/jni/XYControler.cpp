...
void XYControler::setControlValue(int contol, float val) {
    for (map<string, XYFx *>::iterator iter = mapCtrlXY1[contol].begin();
         iter != mapCtrlXY1[contol].end(); ++iter) {
        XYFx *fx = iter->second;
        mListener->setFx(fx->channelIndex, fx->keyFx, fx->keyFxParam,
                         fx->start + val * (fx->end - fx->start));
    }
}

void
XYControler::addFx(string indexControl, int control, int indexChannel, int keyfx, int keyFxParam,
...
void XYControler::clearAllSendMod(int channel) {
    for (int i = 0; i < 6; ++i)
        for (map<string, XYFx *>::iterator iter = mapCtrlXY1[i].begin();
             iter != mapCtrlXY1[i].end(); ++iter) {
