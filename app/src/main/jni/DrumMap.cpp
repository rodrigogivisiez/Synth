...
const int DrumMap::mColors[NUM_CHANNELS] = {0xff6666ff, 0xff6699ff, 0xff66ccff, 0xff66ffff,
                                            0xff66ffcc, 0xff66ff66, 0xff66cc00, 0xffffff00,
                                            0xffffcc00, 0xffff6600, 0xffff0000, 0xffff0066,
                                            0xffff00cc, 0xffff00ff, 0xff6600cc, 0xff6600ff};

void
editWaveChannelCallback(void *clientData, SuperpoweredAdvancedAudioPlayerEvent event, void *value) {
...
bool DrumMap::process(short int *audioIO, unsigned int numberOfSamples, int sampleRate) {
    ...
    for (int i = 0; i < NUM_CHANNELS; ++i) {
        ...
        if (currentChannelProccessing[i] == QSTART || currentChannelProccessing[i] == QASK_STOP) {
            ...
            if (buffer != NULL) {
                ...
                mInputLevel[numChannelProccessing / 4][(numChannelProccessing % 4) *
                                                       2] = mChannels[i]->getLeft();
                mInputLevel[numChannelProccessing / 4][(numChannelProccessing % 4) * 2 +
                                                       1] = mChannels[i]->getRight();
    ...
    for (int i = 0; i < limit; ++i) {
        mMixer->process(mbufferProccessing[i], mSubStereoBuffersOutput[i], mInputLevel[i],
                        mOutputLevel, mInputMeters[i], NULL, numberOfSamples);


        tempBuffer[i] = mSubStereoBuffersOutput[i][0];
    }
    for (int j = 0; j < numChannelProccessing; ++j) {
        mChannels[mProccessIndexOrder[j]]->mViewMeterLeft =
                mInputMeters[j / 4][(j * 2) % 8] * mChannels[mProccessIndexOrder[j]]->getLeft();
        mChannels[mProccessIndexOrder[j]]->mViewMeterRight = mInputMeters[j / 4][(j * 2) % 8 + 1] *
                                                             mChannels[mProccessIndexOrder[j]]->getRight();
    }
    if (numChannelProccessing != 0)
        mMainMixer->process(tempBuffer, mMainStereoBufferOutput, mMainInputLevel, mOutputLevel,
                            mInputMeters[0], NULL, numberOfSamples);
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "processtime", "process time = %f",
                        now_ms() - startProcessMs);
    pthread_mutex_unlock(&mutex);
...
DrumMap::DrumMap(int sampleRate, int bufferSize, void (*funcCallBack)(CallBackJavaTypeMessage, int),
                 const char *tempRecordFile) {
    ...
    mIsQuantize = false;
    mEditPlayer = new SuperpoweredAdvancedAudioPlayer(this, editWaveChannelCallback, mSampleRate,
                                                      0);
    xyControler = new XYControler(this);
    mMainGridControler = new MainGridControler(bufferSize, sampleRate, this);
    mAudioSystem = new SuperpoweredAndroidAudioIO(sampleRate, bufferSize, true, true,
...
void
DrumMap::sendFxToLFO(string indexLfo, int indexChannel, int fxType, int keyEffectParam, int lfo1,
                     int lfo2, double start, double end, bool isUpdate) {
    pthread_mutex_lock(&mutex);
    mChannels[indexChannel]->sendFxToLFO(indexLfo, fxType, keyEffectParam, lfo1, lfo2, start, end,
                                         isUpdate);
    pthread_mutex_unlock(&mutex);
}

void DrumMap::sendFxToControlXY(string mIndexControl, int control, int indexChannel, int mFxType,
                                int keyEffectParam, double start, double end, bool isUpdate) {
    pthread_mutex_lock(&mutex);
    xyControler->addFx(mIndexControl, control, indexChannel, mFxType, keyEffectParam, start, end,
                       isUpdate);
    pthread_mutex_unlock(&mutex);
}
...
void
DrumMap::setValueSequencerPattern(int indexChannel, int indexPattern, int indexValue, double val) {
    pthread_mutex_lock(&mutex);
...
void DrumMap::sendFxToADSR(int indexChannel, string mIndexControl, int fxType, int keyEffectParam,
                           double start, double end, bool isUpdate) {
    mChannels[indexChannel]->sendFxToADSR(mIndexControl, fxType, keyEffectParam, start, end,
                                          isUpdate);
}

void DrumMap::sendFxToCurve(string indexString, int indexChannel, int curve, int fxType,
                            int keyEffectParam, double start, double end, bool isUpdate) {
    mChannels[indexChannel]->sendFxToCurve(indexString, curve, fxType, keyEffectParam, start, end,
                                           isUpdate);
}
...
void DrumMap::proccessEditWave() {
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "proccessEditWave", "durationMs %d buffSize %d",
                        mEditPlayer->durationMs, sizeBuffer);
    ...
    do {
        if (mEditPlayer->process(mTempEditBuffer, false, mBufferSize, 1)) {
            ...
            __android_log_print(ANDROID_LOG_DEBUG, "ndk", "counter %d buffSize %d", counter,
                                sizeBuffer);
        } else if (stateEdit == EDIT_PROCCESS) {
            stateEdit = EDIT_FINISH;
        }
        __android_log_print(ANDROID_LOG_DEBUG, "proccessEditWave", "counter %d buffSize %d",
                            counter, sizeBuffer);
