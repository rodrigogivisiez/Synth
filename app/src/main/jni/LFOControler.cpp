...
void LFOControler::proccess(double distanceMs) {
    ...
    for (map<string, LfoFx *>::iterator iter = mLFOSend.begin(); iter != mLFOSend.end(); ++iter) {
        ...
        mListener->setFx(lfofx->keyFx, lfofx->keyFxParam,
                         lfofx->start + val * (lfofx->end - lfofx->start));
    }
    sample += distanceMs;
    sample %= mSampleRate;
...
void
LFOControler::addFx(string indexLfo, int keyFx, int keyFxParam, int lfo1, int lfo2, double start,
