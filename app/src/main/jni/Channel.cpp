...
static void
pplayerEventCallback(void *clientData, SuperpoweredAdvancedAudioPlayerEvent event, void *value) {
...
float *Channel::internalProccess(double currentPosition) {
    ...
    if (adsr > 0.0) {//__android_log_print(ANDROID_LOG_DEBUG, "ndk", "mPositionMs %f", mEnvA);


        bool isProcess = mPlayer->process(mBufferProccess, false, mBufferSize,
                                          mVolume * adsr * mMainVolume);
...
void Channel::keyDown() {
    ...
    if (mSeqState == SEQ_ON) {
        ...
        mPlayer->setPitchShiftCents((mPitch + mMainPitch + mFxPitch) * 100.0);
        __android_log_print(ANDROID_LOG_DEBUG, "ndk", "setPitchShiftCents %f",
                            (mPitch + mMainPitch + mFxPitch) * 100.0);
        mKeyDown = true;


        if (mIsPlaynigFromStartOnKeyDown || mPlayer->positionMs >= mEndPosition -
                                                                   mUnitLoopProccessMs *
                                                                   NUM_UNIT_PRCS_REMOVE_FROM_END ||
            mPianoRoll->isPlay())
            mPlayer->setPosition(mStartPosition, false, false);
...
void Channel::playerEventCallback(SuperpoweredAdvancedAudioPlayerEvent event, void *value) {
    if (event == SuperpoweredAdvancedAudioPlayerEvent_LoadSuccess) {
        ...
        mStartPosition = 0;
        mEndPosition = mPlayer->durationMs - mUnitLoopProccessMs *
                                             NUM_UNIT_PRCS_REMOVE_FROM_END;// - (((double) mBufferSize * 16000.0) / (double) mSampleRate);
...
long Channel::getDurationMs() {
    return mPlayer->durationMs - mUnitLoopProccessMs *
                                 NUM_UNIT_PRCS_REMOVE_FROM_END;//- (((double) mBufferSize * 16000.0) / (double) mSampleRate);
}
...
void Channel::sendFxToLFO(string indexLfo, int fxType, int keyEffectParam, int lfo1, int lfo2,
                          double start, double end, bool isUpdate) {
    mLFOControler->addFx(indexLfo, fxType, keyEffectParam, lfo1, lfo2, start, end, isUpdate);
}

void Channel::sendFxToADSR(string mIndexControl, int fxType, int keyEffectParam, double start,
                           double end, bool isUpdate) {
    mADSRFxControler->addFx(mIndexControl, fxType, keyEffectParam, start, end, isUpdate);
}

void
Channel::sendFxToCurve(string indexString, int curve, int fxType, int keyEffectParam, double start,
                       double end, bool isUpdate) {
